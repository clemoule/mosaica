...
def Eq_ME_SOL_Parc(m, p, c):
    return m.X[p, "ME"] <= (
        (m.Data_Parc_Gwad[p, "TYPE_SOL"] in [2, 3, 4])
        or (m.Data_Parc_Gwad[p, "ILE"] == 1)
    )
...
@register("Eq_BA_JA")
def Eq_BA_JA(m, e):
    return sum(m.X[p, "JA"] for p in m.parcels[e]) >= m.params["PROP_BA_JA"] * sum(
        m.X[p, "BA"] for p in m.parcels[e]
    )
